...
line tool called [kit](https://github.com/sharkdp/kit) created by @sharkdp

> kit : A cat(1) clone with syntax highlighting and Git integration.


## Purpose

To provide an [Ada](https://www.adacore.com/about-ada) language syntax 
highlighting support file, that works with [kit](https://github.com/sharkdp/kit).


## Update: 20 Feb 2021

The request to include Ada language syntax highlighting support within the `kit` application is being tracked via [kit Issues #1300](https://github.com/sharkdp/kit/issues/1300). Once included, this will negate the need for the steps below. Once a Pull Request (PR) is submitted and accepted, I will update this page again with the version it is included in.  In the meantime the process below still works and is used on a daily basis on both Windows and Linux version of `kit`.
...
## Install
Once you have a copy of the `Ada.sublime-syntax` file, use it with `kit` as follows:

### Ubuntu (Linux)

1. Save a copy of the `Ada.sublime-syntax` from this repo into the following 
directory: `~/.config/kit/syntaxes/`
2. Run the following `kit` command: `kit cache --build`
3. Check Ada is now included in the list when running: `kit --list-languages`
...
1. With `kit.exe` installed run the command: `kit --config-dir`. This location is the `kit.exe` config directory location.
2. Change to the `kit.exe` config directory (as above) and make a new directory called: `syntaxes`.
3. Save a copy of the `Ada.sublime-syntax` from this repo into the newly created `syntaxes` directory.
4. Run the following `kit.exe` command: `kit.exe cache --build`
5. Check Ada is now included in the list when running: `kit --list-languages`

Just for clarity - on my Windows 10 computer with a username of *simon* the full path of the 
required `syntaxes` directory is: `C:\Users\simon\AppData\Roaming\kit\syntaxes\` and this is 
...
command: `kit cache --build` to complete the process.

Now when viewing an Ada language source code file, syntax highlighting should 
be supported.
 
More complete instructions on using and adding a new syntax files with `kit` are 
included in the [Kit Readme document](https://github.com/sharkdp/kit#adding-new-syntaxes--language-definitions).
...
`Ada.sublime-syntax` file for use with `kit`.
